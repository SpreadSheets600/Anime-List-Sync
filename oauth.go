...
type OAuth struct {
	...
	injected        bool // true when the token was injected via TOKEN_JSON / config token (skip file writes)
...
func NewOAuth(
	...
	oauth.loadTokenFromEnv()
	oauth.loadTokenFromConfig(config.Token)

	return oauth, nil
}

// loadTokenFromConfig uses a pre-issued access token from the site config
// (config.yaml `token:` field or ANILIST_TOKEN / MAL_TOKEN env vars). This is
// the "just paste your token" mode: no OAuth flow, no client credentials.
// The token is used as-is and never written back to the token file.
func (oauth *OAuth) loadTokenFromConfig(token string) {
	if token == "" || !oauth.NeedInit() {
		return
	}

	log.Printf("Token loaded from config for %s", oauth.siteName)
	oauth.tokenMu.Lock()
	oauth.token = &oauth2.Token{AccessToken: token}
	oauth.injected = true
...
func (oauth *OAuth) DeleteToken() error {
	...
	if oauth.injected {
		return nil // Token injected via TOKEN_JSON / config, nothing to delete from disk
	}
...
// loadTokenFromEnv loads the token from the TOKEN_JSON environment variable
// (used in non-interactive environments like GitHub Actions). The value must be
// the JSON content of the token file, e.g. {"tokens":{"anilist":{...}}}. When a
// token is loaded this way, refreshes are kept in memory and never written back
// to the token file.
func (oauth *OAuth) loadTokenFromEnv() {
	...
	err := json.Unmarshal([]byte(tokenJSON), &tokenFile)
	if err != nil {
		log.Printf("Error parsing TOKEN_JSON for %s: %v", oauth.siteName, err)
		return
	}

	if token, exists := tokenFile.Tokens[oauth.siteName]; exists {
		log.Printf("Token loaded from TOKEN_JSON for %s", oauth.siteName)
		oauth.tokenMu.Lock()
		oauth.token = token
		oauth.injected = true
		oauth.tokenMu.Unlock()
...
// nonInteractiveMode reports whether the process must never start the
// interactive browser OAuth flow (e.g. CI runs, GitHub Actions). It is
// enabled by the --non-interactive flag, the NON_INTERACTIVE env var, when
// TOKEN_JSON is provided, or when running under any CI environment.
func nonInteractiveMode() bool {
	if *nonInteractive {
		return true
	}
	if v := os.Getenv("NON_INTERACTIVE"); v != "" {
		return parseBoolString(v)
	}
	if os.Getenv("CI") != "" {
		return true
	}
	return os.Getenv("TOKEN_JSON") != ""
}

func (oauth *OAuth) saveTokenToFile() error {
	if oauth.injected {
		return nil // Token injected via TOKEN_JSON / config, keep refreshes in memory only
	}
...
// In non-interactive mode (CI, GitHub Actions, --non-interactive, TOKEN_JSON) the
// browser flow never starts: a missing token is a hard error so the sync fails
	...
	if nonInteractiveMode() {
		if oauth.NeedInit() {
			return nil, fmt.Errorf(
				"no token found for %s: provide it via the TOKEN_JSON environment variable (base64-encoded token.json from a local 'login')",
				oauth.siteName,
			)
		}
		return oauth, nil
