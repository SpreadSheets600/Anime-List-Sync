...
var syncFlags = []cli.Flag{
	...
	&cli.BoolFlag{
		...
		Usage: "never start the browser OAuth flow; fail if tokens are missing (auto-enabled in CI / when TOKEN_JSON is set)",
	},
	&cli.StringFlag{
		Name:  "stats-file",
		Usage: "write a markdown sync stats block to this file (for README badges / GitHub Actions)",
	},
}
...
func getSyncFlagsFromCmd(cmd *cli.Command) (verboseOut bool, reverseOut bool) {
	...
	nonInteractiveVal := cmd.Bool("non-interactive")
	...
	allSync = &allVal
	verbose = &verboseVal
	nonInteractive = &nonInteractiveVal

	if cmd.IsSet("stats-file") {
		statsFileVal := cmd.String("stats-file")
		statsFile = &statsFileVal
	} else if envPath := os.Getenv("STATS_FILE_PATH"); envPath != "" {
		statsFile = &envPath
	}
