...
func TestCLI_HasFlags(t *testing.T) {
	...
	if len(cmd.Flags) != 15 {
		t.Errorf("expected 15 flags on root command, got %d", len(cmd.Flags))
	}
...
func TestCLI_SyncCommand_HasFlags(t *testing.T) {
	...
	if len(syncCmd.Flags) != 14 {
		t.Errorf("expected 14 flags on sync command, got %d", len(syncCmd.Flags))
	}
...
func TestCLI_WatchCommand_HasSyncFlags(t *testing.T) {
	...
	// watch has 3 own flags (interval, schedule, once) + 14 sync flags = 17 total
	if len(watchCmd.Flags) != 17 {
		t.Errorf("expected 17 flags on watch command (3 watch + 14 sync), got %d", len(watchCmd.Flags))
	}
