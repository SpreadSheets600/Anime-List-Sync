...
type SiteConfig struct {
	...
	// Token is a pre-issued access token (e.g. generated by the GitHub Pages
	// setup site). When set, no OAuth flow is started and no client credentials
...
func overrideAnilistFromEnv(anilist *SiteConfig) {
	...
	overrideStringFromEnv(&anilist.Token, "ANILIST_TOKEN")
}
...
func overrideMyAnimeListFromEnv(mal *SiteConfig) {
	...
	overrideStringFromEnv(&mal.Token, "MAL_TOKEN")
}
...
// validateConfig checks that each site has a username and at least one way to
// authenticate: a client ID (OAuth) or a pre-issued token (token mode).
func validateConfig(cfg Config) error {
	if cfg.Anilist.Username == "" {
		return errors.New("anilist.username not set (ANILIST_USERNAME)")
	}
	if cfg.MyAnimeList.Username == "" {
		return errors.New("myanimelist.username not set (MAL_USERNAME)")
	}
	if cfg.Anilist.ClientID == "" && cfg.Anilist.Token == "" {
		return errors.New("anilist credentials not set (ANILIST_CLIENT_ID or ANILIST_TOKEN)")
	}
	if cfg.MyAnimeList.ClientID == "" && cfg.MyAnimeList.Token == "" {
		return errors.New("myanimelist credentials not set (MAL_CLIENT_ID or MAL_TOKEN)")
	}
	return nil
}
...
func loadConfigFromFile(filename string) (Config, error) {
	...
	if err := validateConfig(cfg); err != nil {
		return Config{}, fmt.Errorf(
			"invalid config: %w (anilist: client_id or token + username; myanimelist: client_id or token + username)",
			err,
		)
	}
	...
	if err := validateConfig(cfg); err != nil {
		return Config{}, fmt.Errorf(
			"required environment variables not set: %w"+
				" (ANILIST_USERNAME, MAL_USERNAME, and ANILIST_CLIENT_ID/ANILIST_TOKEN,"+
