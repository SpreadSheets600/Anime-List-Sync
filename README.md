...
Run syncs entirely on GitHub's infrastructure with a scheduled workflow. This repository is a
...
click **Use this template** to create your own repo, configure secrets, and the included workflow
will sync your lists daily (04:00 UTC by default) and rewrite a stats block in your README.
...
The easiest way to get tokens: the setup site hosted on Pages walks you through the
whole flow in the browser.
...
- **OAuth mode (TOKEN_JSON):** the app refreshes expired access tokens automatically using your
  client secret — no re-authentication needed. To set this up, run `anilist-mal-sync login` once
  locally (interactive OAuth) and store `base64 -w0 ~/.config/anilist-mal-sync/token.json` in the
  `TOKEN_JSON` secret.
...
| | `--non-interactive` | Never start the browser OAuth flow; fail if tokens are missing (auto-enabled in CI / when `TOKEN_JSON` is set) |
| | `--stats-file` | Write a markdown sync stats block to this file (for README badges / GitHub Actions) |
...
**Required (either auth mode):**
- `ANILIST_USERNAME` - AniList username
- `MAL_USERNAME` - MyAnimeList username

**Auth mode 1 - pre-issued tokens (no OAuth flow, recommended):**
- `ANILIST_TOKEN` - AniList access token (from the setup site)
- `MAL_TOKEN` - MyAnimeList access token

**Auth mode 2 - OAuth with auto-refresh:**
...
Either `ANILIST_TOKEN`/`MAL_TOKEN` or the client credentials are required per service; a pre-issued
...
- `TOKEN_JSON` - JSON token file content (`{"tokens":{...}}`) for non-interactive runs; tokens are loaded from here instead of the token file and refreshes are kept in memory (CI-friendly). See [GitHub Actions Deployment](#github-actions-deployment-template)
- `NON_INTERACTIVE` - Set to `true` to never start the browser OAuth flow; missing tokens become a hard error (auto-enabled when `CI` or `TOKEN_JSON` is set)
- `STATS_FILE_PATH` - Write a markdown sync stats block to this path after each sync (equivalent to `--stats-file`)
