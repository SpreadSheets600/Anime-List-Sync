...
// WriteStatsFile writes a markdown sync stats block to the given file.
// The block is wrapped in HTML comment markers so CI tooling can splice it
// into a README (see .github/update_readme_stats.py). Written even on sync
// failure so the workflow can reflect the failure in the README.
func WriteStatsFile(path string, updaters []*Updater, report *SyncReport, totalDuration time.Duration, reverse bool, runErr error) error {
	var b strings.Builder

	b.WriteString("<!-- SYNC-STATS:START -->\n")
	b.WriteString("_Automatically updated by the anilist-mal-sync GitHub Actions workflow._\n\n")

	if runErr != nil {
		b.WriteString("**Last sync: FAILED** - check the Actions run log for details.\n\n")
	} else {
		fmt.Fprintf(&b, "**Last sync:** %s UTC (%s) - duration %s\n\n",
			time.Now().UTC().Format(time.RFC3339), DirectionOf(reverse).Label(), totalDuration.Round(time.Millisecond))
	}
	...
	if totals.dryRun > 0 {
		fmt.Fprintf(&b, "| Dry run (would update) | %d |\n", totals.dryRun)
	}
	if report != nil {
		fmt.Fprintf(&b, "| Unmapped | %d |\n", len(report.UnmappedItems))
		fmt.Fprintf(&b, "| Favorites added | %d |\n", report.FavoritesAdded)
		if report.HasFavoritesMismatches() {
			fmt.Fprintf(&b, "| Favorites mismatches | %d |\n", len(report.FavoritesMismatches))
		}
	}
	b.WriteString("<!-- SYNC-STATS:END -->\n")
